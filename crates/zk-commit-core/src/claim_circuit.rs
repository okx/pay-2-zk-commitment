use plonky2::{hash::hash_types::HashOutTarget, iop::{target::Target, witness::{PartialWitness, WitnessWrite}}, plonk::circuit_builder::CircuitBuilder};

use crate::{circuit_config::{COMMITMENT_TREE_DEPTH, D}, circuit_utils::{get_hash_from_input_targets, verify_hash, verify_merkle_proof_fixed_height}, claim_execution::ClaimProvingInputs, types::F};
...
pub fn generate_claim_circuit(
    ...
    verify_merkle_proof_fixed_height(
        builder,
        commitment,
        own_leaf_hash_calculated,
        &siblings
    );
...
pub fn set_claim_circuit(
    ...
    assert_eq!(claim_proving_inputs.siblings.len(), COMMITMENT_TREE_DEPTH-1);

    for i in 0..COMMITMENT_TREE_DEPTH-1{
        pw.set_hash_target(*claim_targets.siblings.get(i).unwrap(), *claim_proving_inputs.siblings.get(i).unwrap());
    }
